...
func (r *Reader) OutlineWithPage() OutlineWithPage {
	...
	pageMap := make(map[objptr]int)
	rootPages := r.Trailer().Key("Root").Key("Pages")
	r.collectPagePtrs(rootPages, pageMap, 1)

	return r.buildOutlineWithPage(root, pageMap)
}

func (r *Reader) collectPagePtrs(v Value, pageMap map[objptr]int, pageNum int) int {
	if v.Key("Type").Name() == "Pages" {
		kids := v.Key("Kids")
		for i := 0; i < kids.Len(); i++ {
			pageNum = r.collectPagePtrs(kids.Index(i), pageMap, pageNum)
		}
	} else if v.Key("Type").Name() == "Page" {
		pageMap[v.ptr] = pageNum
		pageNum++
	}
	return pageNum
}

func (r *Reader) PageNumFromRef(pageRef Value, pageMap map[objptr]int) int {
	return pageMap[pageRef.ptr]
}

func (r *Reader) buildOutlineWithPage(entry Value, pageMap map[objptr]int) OutlineWithPage {
	...
	if dest.Kind() == Array && dest.Len() > 0 {
		...
		outline.Page = r.PageNumFromRef(pageRef, pageMap)
	} else {
		action := entry.Key("A")
		if action.Kind() == Dict {
			d := action.Key("D")
	...
	for child := entry.Key("First"); child.Kind() == Dict; child = child.Key("Next") {
		childOutline := r.buildOutlineWithPage(child, pageMap)
